			...
			*/
			filter: function(targetFilter) {
				...
				*/
				...
				else if(self._activeArray && self._activeArray.length > 0) {
					self._shouldSetNoResultsClass(false);
				}
			},
			...
			search: function(text) {
				...
				//If there are no results
					...
					if (self._isSearchActivated()) {
						...
						//	Note, we now add the class at the transition end instead of doing it here...
						//self._shouldSetNoResultsClass(true);
					}
		...
		var FiltrItemProto = {
			...
			_onTransitionEnd: function() {
				...
				if (self._filteringOut) {
					if(!self._filteringIn) {
						//TJM: Searching with filtering makes things a bit more complex and incorrect, so adding this extra check.
						$(self).addClass('filteredOut');
					}
					self._isFilteredOut = true;
					self._filteringOut  = false;
				...
				if(filtrItems.length === filtrItemsOut.length) {
					...
					self._parent._shouldSetNoResultsClass(true);
				}
			},
			...
			*/
			_filterOut: function() {
				...
				// 	//TJM: Don't need to filter out an item that is already filtered out.
				// 	// But for certain cases where we might filter something while we have text in the search box...
				// 	//	(search happens after items get filtered so we should make sure to set this property even though an item might be filtered out and waiting to get filtered in)
				// 	//Tag as filteringOut for transitionend event
				// 	this._filteringOut = true;
				// 	return;
				// }
